...
Rust genome assembler: correctness and resource use are first-class; biological scope expands in deliberate phases. **Active engineering** is Illumina **Phase-1** until an explicit replan; **Phase-2** is captured in **Phase-2 target**, the consecutive **Phase-2 Illumina** glossary entries from **Phase-2 Illumina primary export** through **Phase-2 Illumina scope and CI cadence**, and **Phase-2 deferral** (long-read sequencing and overall Phase-2 bounds). Further Phase-2 detail ships via grilling, **`ARCHITECTURE.md`**, or **ADRs**, not unbounded glossary growth.
...
Scheduling and implementation attention stay on **Phase-1 target** Illumina short-read assembly until deliberately replanned; long-read ingest and **Phase-2** integration are not active workstreams during that period.
_Avoid_: Treating Phase-2 long-read milestones as parallel deliverables while claiming Illumina-only focus (for current bandwidth)
