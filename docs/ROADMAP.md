...
| Simplification policy | Active | Decision-first tip clipping, bounded diamond bubble records, short low-copy component pruning, repeat-aware high-copy diamond retention, Phase-2 balanced bubble retention, `simplification.json` schema v3. | Add projected-bulge provenance and non-noop recompression before stronger repeat/path cleanup behavior changes. |
...
| SPAdes architecture lane | Active | [`docs/SPADES_ARCHITECTURE_INSPIRATION.md`](SPADES_ARCHITECTURE_INSPIRATION.md), read-trust diagnostics, multi-*k* evidence, k-bimer-like mate constraints, promotion policy, scaffold paths, repeat-aware simplification notes, and low-copy component cleanup. | Build read-span trust evidence, projected-bulge provenance, and path-first repeat resolution before mutating primary FASTA semantics. |
...
| Simplification policy | Tip, bubble, and component passes expose decision records while preserving sparse default assemblies. | `simplification.json` schema v3 with `spades_iterative_v1` scheduler metadata, decision/mutation equivalence tests, component-pruning guardrail tests, scheduler unit test, PR smoke, and Drosophila 1M component-pruning artifact. | Proven for tip/diamond/component passes and topology-delta reporting; N50 remains poor and non-noop recompression/path resolution remain future work. |
