...
| Evidence ledger | Active | `trex::evidence`, `evidence.json`, mate-bridge evidence records, conflict-aware endpoint join acceptance/rejection, mate orientation/distance confidence, and `xtask bench` JSON embedding. | Add richer conflict provenance before allowing any scaffold output to mutate primary artifacts. |
...
| Path/scaffold builder | Active | Unitigs, primary contigs, GFA `S`/`L`/representable `P` rows, mate-backed `scaffolds.json` schema v5 with promotion-policy snapshots, accepted/rejected endpoint joins, orientation/distance evidence, contig endpoint fragmentation diagnosis, high-confidence endpoint sidecar path promotion, separate `scaffolds.fa` output for accepted paths, and tagged `scf...` GFA path projection. | Add richer conflict provenance and larger-row scaffold artifact benchmarks before any primary FASTA mutation. |
...
| SPAdes architecture lane | Active | [`docs/SPADES_ARCHITECTURE_INSPIRATION.md`](SPADES_ARCHITECTURE_INSPIRATION.md), multi-*k* evidence, promotion policy, scaffold paths, repeat-aware simplification notes. | Build read-trust diagnostics, k-bimer-like mate constraints, and iterative simplification as typed artifacts before mutating graph topology. |
...
| Bridge and scaffold artifact | Mate evidence produces deterministic scaffold/path sidecars without mutating primary FASTA. | `scaffolds.json` schema v5, `scaffolds.fa`, promotion-policy snapshot tests, existing-edge scaffold tests, orientation/distance evidence tests, accepted/rejected endpoint join tests, conflict-cluster rejection tests, and primary-contig smoke regression. | Proven for explicit existing-edge bridges, high-confidence absent-edge sidecar path promotion, separate scaffold FASTA emission, and conflict-cluster rejection. |
