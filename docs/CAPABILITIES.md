...
|------------|-----------------|-----------------------------|-------------------|
| Read technology | Illumina single-end or paired-end via `--r1` and optional `--r2`; FASTQ/FASTA, including gzip by suffix, parsed through streaming readers before normalized reads are retained for graph construction. | Same Illumina ingest path; no separate diploid subcommand. | Long-read, hybrid, HiFi, CLR, and ONT paths remain deferred by `CONTEXT.md` and ADRs. |
...
| Biological data fetch | `cargo run -p xtask -- fetch-data` | manual/local | Prepares ignored `data/benchmarks/` FASTQ subsets, public reference FASTAs, derived reference slices, interval-paired FASTQs from indexed BAMs, and local Benchmarks repo symlinks declared in `tools/benchmark_data.toml`, then verifies prepared SHA-256s where pinned. External `curl`, `gzip`, and `samtools` work is composed with argv/pipes rather than shell-interpolated catalog fields. `validate-data` streams SHA-256 checks and skips very large local files by default; set `TREX_XTASK_VALIDATE_DATA_FULL_HASH=1` for full local digest verification. |
...
`graph/simplified_dbg.json`, `graph/stage_artifacts.json`, and
`export/sequences.json`. Leave the variable
