...
| Outputs | `--out-dir`, `--unitigs-fasta`, `--contigs-fasta`, and `--gfa`; defaults are `unitigs.fa`, `contigs.fa`, `graph.gfa`, JSON sidecars, post-assembly `audit.json` / `audit.tsv`, and diploid `diploid.json`; `-` writes selected FASTA/GFA artifacts to stdout. | Same files, with primary FASTA collapse and GFA `XX:Z:trex-phase2-illumina`, optional `L` rows, `ctg...` `P` rows, `p2h...` mirror paths where representable, parent-specific path tags when `--parent1-reference` / `--parent2-reference` are provided, and audit reports that do not repair FASTA. | GFA 2, mandatory dual haplotype FASTA, and audit-driven sequence repair are deferred. |
