...
| Counting and trust | `--kmer-size` / `-k`, optional explicit `--kmer-ladder`, `--trusted-threshold` / `-T`; canonical *k*-mer counts and one global trusted threshold. Without `--kmer-ladder`, exactly one requested graph is built. With `--kmer-ladder`, candidate graphs are scored independently and one graph is selected with `multi_k.json` evidence. | Counting and trusted membership are unchanged before the graph exists. | PE-informed trust, local thresholds, and graph merging across *k* values need a new scope decision. |
...
| Mate usage | Paired reads are validated and concatenated after parity checks; no default mate-derived graph edits. | `--insert-mean-bp` plus paired input enables conservative boosts on existing DBG edges and report-only dead-end endpoint join ranking for absent edges; `scaffolds.json` records orientation, estimated gap, distance confidence, support, and conflict counts; `--insert-stddev-bp` is stored in mode identity and contributes to confidence. | Mate-derived new edges, scaffold gaps, and distance-sensitive surgery are out of scope. |
| Outputs | `--out-dir`, `--unitigs-fasta`, `--contigs-fasta`, and `--gfa`; defaults are `unitigs.fa`, `contigs.fa`, `graph.gfa`, JSON sidecars, contig endpoint `fragmentation.json`, post-assembly `audit.json` / `audit.tsv`, and diploid `diploid.json`; `-` writes selected FASTA/GFA artifacts to stdout. | Same files, with primary FASTA collapse and GFA `XX:Z:trex-phase2-illumina`, optional `L` rows, `ctg...` `P` rows, `p2h...` mirror paths where representable, parent-specific path tags when `--parent1-reference` / `--parent2-reference` are provided, fragmentation diagnosis that does not alter contigs, and audit reports that do not repair FASTA. | GFA 2, mandatory dual haplotype FASTA, and audit-driven sequence repair are deferred. |
| Checkpoints | `--checkpoint-root`, `--resume`, `--no-resume`, `--strict-checkpoints`, and `--no-strict-checkpoints`; graph/export reload by documented identity. Explicit multi-*k* mode rejects checkpoints until selected-*k* checkpoint namespacing lands. | Graph checkpoint identity includes diploid mode, paired input, insert priors, and mate-bridge version. | Cross-mode resume that silently reuses stale graph shape is not allowed. |
...
| Automatic k selection | `trex illumina assemble --auto-k --r1 reads.fq --out-dir run` | manual/local | Derives a deterministic odd-*k* ladder from retained read length, scores it through the multi-*k* selector, writes `multi_k.json`, and logs the selected *k* for auditability. |
...
| Drosophila Benchmarks comparison row | `cargo run -p xtask -- bench --tier manual --row drosophila_illumina_pe_1m_benchmarks_comparison --out target/benchmarks/drosophila-illumina-1m.json` | manual/local | Runs release Trex with `--diploid` on the same local `/home/jake/Projects/Benchmarks` Drosophila melanogaster DRR001444 1M-pair Illumina rung used for SPAdes, MEGAHIT, Velvet, and Tadpole comparison work. |
