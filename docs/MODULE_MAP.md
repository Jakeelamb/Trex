...
| 7 | Mate evidence | `illumina/mate.rs` | Mate bridge candidates, support, conflicts, distance confidence; future k-bimer-like graph-context constraints. | Insert mean/stddev, support and conflict counts. | Pair scanning and endpoint lookup. | Active |
...
| 9 | Simplification policy | `dbg/simplify.rs` | Tip and diamond decisions, graph edits, guardrails, future edit/recompress/reannotate/replan loop. | Tip length/multiplicity, bubble node/base budgets, diploid/repeat retention. | Graph mutation complexity, recompression cost, and decision-log allocation. | Active |
...
| 13 | Scaffold/path builder | `illumina/scaffold.rs` | `scaffolds.json`, accepted scaffold paths, scaffold FASTA records, GFA path projection. | Promotion decisions, policy snapshots, orientation, overlap trimming, positive-gap `N` padding. | Sequence concatenation and unitig lookup. | Active |
