...
- **Biological manual rows**: `cargo run -p xtask -- fetch-data` prepares ignored ENA/GIAB subsets, public reference FASTAs, derived reference slices, and interval-paired FASTQs from `tools/benchmark_data.toml`; then run a single row with `cargo run -p xtask -- bench --tier manual --row ecoli_mg1655_srr001666_1k_pairs --out target/benchmarks/ecoli.json`, `--row ecoli_mg1655_srr001666_10k_pairs`, `--row yeast_btt_err1308583_diploid_1k_pairs`, or `--row hg002_giab_grch38_chr20_interval_1k_pairs`.
